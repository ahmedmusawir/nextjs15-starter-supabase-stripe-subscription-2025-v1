...
create policy "auth read baseline"
  on public.pharma_baseline
  for select to authenticated
  using (true);

create policy "auth read alt_rates"
  on public.pharma_alt_rates
  for select to authenticated
  using (true);

create policy "auth read pbm"
  on public.pharma_pbm_info
  for select to authenticated
  using (true);

-- Membership visibility
create policy "member sees self"
  on public.pharma_pharmacy_members
  for select to authenticated
  using (user_id = auth.uid());

create policy "member self-insert"
  on public.pharma_pharmacy_members
  for insert to authenticated
  with check (user_id = auth.uid());

-- Pharmacy profile access
create policy "profile select by membership"
...
create policy "profile insert by membership"
  on public.pharma_pharmacy_profile
  for insert to authenticated
  with check (
    exists (
...
create policy "profile update by membership"
  on public.pharma_pharmacy_profile
...
-- User data policies
create policy "userdata select by membership"
...
create policy "userdata insert by membership"
  on public.pharma_user_data
  for insert to authenticated
  with check (
    exists (
...
create policy "userdata update by membership"
  on public.pharma_user_data
  for update to authenticated
  using (
    exists (
...
-- Report file policies
create policy "reports select by membership"
...
create policy "reports insert by membership"
  on public.pharma_report_files
  for insert to authenticated
  with check (
    exists (
